...
def _load_profile(profile: str) -> dict[str, Any]:
    ...
    if not p.exists():
        raise FileNotFoundError(f"Systema ref_compare profile not found: {p}")
    obj = _load_profile_yaml(p)
    unknown = sorted(set(obj.keys()) - PROFILE_ALLOWED_KEYS)
    if unknown:
        raise ValueError(f"Unknown keys in Systema ref_compare profile {p.name}: {unknown}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--profile", default="", help="dataset profile under scripts/systema/ref_compare/configs")
    ap.add_argument("--sweep-dir", required=False, help="Sweep root directory containing run subfolders.")
    ...
    if profile:
        ...
        if not args.sweep_dir and task_args.get("sweep_dir") not in (None, ""):
            args.sweep_dir = str(task_args["sweep_dir"])
        if args.out_root is None and task_args.get("out_root") not in (None, ""):
            args.out_root = str(task_args["out_root"])
        if args.splits is None and task_args.get("splits"):
            args.splits = ",".join([str(x) for x in list(task_args["splits"])])
        if bool(task_args.get("umap", True)) is False and not args.no_umap:
            args.no_umap = True
        if args.umap_pca_dim == 50 and "umap_pca_dim" in task_args:
            args.umap_pca_dim = int(task_args["umap_pca_dim"])
        if args.umap_n_neighbors == 15 and "umap_n_neighbors" in task_args:
            args.umap_n_neighbors = int(task_args["umap_n_neighbors"])
        if abs(float(args.umap_min_dist) - 0.3) < 1e-12 and "umap_min_dist" in task_args:
            args.umap_min_dist = float(task_args["umap_min_dist"])
        if args.umap_random_state == 0 and "umap_random_state" in task_args:
            args.umap_random_state = int(task_args["umap_random_state"])

    if not args.sweep_dir:
        raise ValueError("Either --sweep-dir or profile task_args.sweep_dir must be provided")
