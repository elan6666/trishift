...
def _build_runs() -> list[RunSpec]:
    ...
    add("B0_baseline", {})
    add("OPT1_lr_5e4_gamma1", {"train": {"stage23": {"lr": 0.0005, "sched_gamma": 1.0}}})
    add("OPT2_lr_1e4_gamma1", {"train": {"stage23": {"lr": 0.0001, "sched_gamma": 1.0}}})
    add("G1_use_residual_head_false", {"model": {"stage3": {"use_residual_head": False}}})
    add(
        "S1_shift_repr_dim_16",
        {"model": {"stage2": {"predict_delta": False, "shift_repr_dim": 16}}},
    )
    add(
        "TR1_transformer_readout_first",
        {
            "model": {
                "stage2": {
    ...
    add(
        "TR2_transformer_readout_mean",
        {
            "model": {
                "stage2": {
                    "use_transformer_block": True,
                    "use_cross_attention": False,
                    "transformer_readout": "mean",
                }
            }
        },
    )
    add(
        "AT1_use_transformer_block_false",
        {"model": {"stage2": {"use_transformer_block": False, "use_cross_attention": False}}},
    )
    add(
        "AT2_cross_attention_true_transformer_false",
        {"model": {"stage2": {"use_transformer_block": False, "use_cross_attention": True}}},
    )
    add(
        "C1_cond_pool_mean_l2_true",
        {"model": {"stage2": {"cond_l2_norm": True, "cond_pool_mode": "mean"}}},
    )
    add("PD1_predict_delta_true", {"model": {"stage2": {"predict_delta": True}}})
    add("L1_lambda_expr_mse_0p2", {"loss": {"lambda_expr_mse": 0.2}})
    add("L2_lambda_dir_expr_0p3", {"loss": {"lambda_dir_expr": 0.3}})
    add(
        "AT3_cross_attn_heads_2",
        {
            "model": {
                "stage2": {
                    "use_transformer_block": True,
    ...
    add(
        "TR4_transformer_ff_mult_8",
        {
            "model": {
                "stage2": {
                    "use_transformer_block": True,
                    "use_cross_attention": False,
                    "transformer_ff_mult": 8,
                }
            }
        },
    )
...
def main() -> None:
    ...
    local_cfg_path = Path(__file__).with_name("config.yaml")
    ...
    parser = argparse.ArgumentParser(
        description="Run TriShift custom single-factor sweep (Norman local, 3-split default)"
    )
    ...
    parser.add_argument(
        "--n_splits",
        type=int,
        default=int(sweep_cfg.get("n_splits", 3)),
        help="global sweep override for run.n_splits",
    )
    parser.add_argument(
        "--suffix",
        default=str(sweep_cfg.get("output_suffix", "custom_sweep_pdFalse_base_no_mutex_n3")),
