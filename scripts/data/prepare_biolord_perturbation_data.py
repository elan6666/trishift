...
def prepare_dataset(
    ...
    *,
    ...
    split_ids = tuple(int(x) for x in (split_ids or cfg.split_ids))
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Split ids to materialize into split*/subgroup* columns. Defaults to each dataset's maintained split ids.",
    )
    ap.add_argument(
        "--overwrite",
        action="store_true",
...
def main(argv: list[str] | None = None) -> None:
    ...
    split_ids = tuple(int(x) for x in args.split_ids) if args.split_ids else None
